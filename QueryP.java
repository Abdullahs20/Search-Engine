public class QueryP {
    ...
    public static LinkedList<Integer>AndQuery(String Query) { //7
        ...
        if (T.length == 0)
            return X;
        boolean Found = Inverted.search_inverted_word_(T[0].trim().toLowerCase()); //search_inverted_word_ you find it in class InvertedIndex , also when we split the new LL will be empty
        if (Found) {
            X = Inverted.WordList.retrieve().DOC_ID;
        }
        for (int i = 1; i < T.length; i++) {
            Found = Inverted.search_inverted_word_(T[i].trim().toLowerCase());
            if (Found)
                Y = Inverted.WordList.retrieve().DOC_ID;
            X = AndQuery(X, Y);
        }
        return X;
    }
    ...
    public static LinkedList<Integer> ORQuery(String Query) {
        ...
        boolean Found = Inverted.search_inverted_word_(T[0].trim().toLowerCase());
        if (Found) {
            A = Inverted.WordList.retrieve().DOC_ID;
        }
        for (int i = 1; i < T.length; i++) {
            Found = Inverted.search_inverted_word_(T[i].trim().toLowerCase());
            if (Found) {
                B = Inverted.WordList.retrieve().DOC_ID;
            }
            A = ORQuery(A, B);
        }
        return A;
